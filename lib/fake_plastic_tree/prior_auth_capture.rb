module FakePlasticTree
  module PriorAuthCapture
    
    def prior_auth_capture(transaction_id, amount)
      hash = hash_from_transaction_id(transaction_id)
      if amount.to_f <= hash[:amount].to_f
        response = response_prior_auth_capture_success(transaction_id, amount)
        transaction_success(response)
      else
        response = response_prior_auth_capture_error(transaction_id, amount)
        any_error(response)
      end
    end
    ...
    def response_prior_auth_capture_success(transaction_to_capture, amount_to_capture)
      time = Time.now
      amount = amount_to_capture
      transaction_id = transaction_to_capture
      prop = hash_from_transaction_id(transaction_to_capture)
      ...
      {:transaction=>
               {:customer=>{:company=>nil, :first_name=>"Jen", :last_name=>"Smith", :email=>nil, :phone=>nil, :id=>customer_id, :website=>nil, :fax=>nil},
                ...
                :avs_postal_code_response_code=>"M", :processor_authorization_code=>"D3042M",
    ...
    def response_prior_auth_capture_error(transaction_to_capture, amount_to_capture)
      amount = amount_to_capture
      transaction_id = transaction_to_capture
      {:api_error_response=>
               {:params=>
                        {:transaction=>{:amount=>amount}, :action=>"submit_for_settlement", :controller=>"transactions", :merchant_id=>merchant_id, :id=>transaction_id},
                :errors=>{:transaction=>{:errors=>[{:attribute=>"amount", :message=>"Settlement amount cannot be more than the authorized amount.", :code=>"91522"}]}, :errors=>[]},
                :message=>"Settlement amount cannot be more than the authorized amount."
               }
      }
    end
